...
export function HomePage() {
  ...
  const { totalBalance, totalIncome, totalExpenses, balanceTrend } = useMemo(() => {
    ...
    const expenses = recentTransactions.filter(tx => tx.type === 'expense').reduce((sum, tx) => sum + tx.amount, 0);
    const balance = accounts.reduce((sum, acc) => sum + acc.balance, 0);
    const sortedTx = [...recentTransactions].sort((a, b) => a.ts - b.ts);
    let currentBalance = balance - sortedTx.reduce((acc, tx) => acc + (tx.type === 'income' ? tx.amount : -tx.amount), 0);
    const trendData = sortedTx.map(tx => {
        currentBalance += tx.type === 'income' ? tx.amount : -tx.amount;
        return { date: new Date(tx.ts).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), balance: currentBalance };
    });
      ...
      <div className="py-8 md:py-10 lg:py-12">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
          ...
          <Button asChild size="lg" className="bg-orange-500 hover:bg-orange-600 text-white">
            <Link to="/transactions"><PlusCircle className="mr-2 size-5" /> Agregar Transacción</Link>
          </Button>
        </header>
                    ...
                    <motion.div key={acc.id} variants={cardVariants}>
                      <Card className="hover:bg-muted/50 transition-colors">
                        <CardContent className="p-4 flex justify-between items-center">
                          <div><p className="font-semibold">{acc.name}</p><p className="text-sm text-muted-foreground">{formatCurrency(acc.balance)}</p></div>
                  ...
                  {loading ? <Skeleton className="h-[250px]" /> : (
                    <ResponsiveContainer width="100%" height={250}>
                      <LineChart data={balanceTrend}>
                        ...
                        <YAxis stroke="#888888" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}`} domain={['dataMin', 'dataMax']} />
                        <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--background))', border: '1px solid hsl(var(--border))' }} cursor={{ stroke: 'hsl(var(--border))', strokeWidth: 2 }} />
